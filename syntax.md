...
`&` 符號其實很讓寫作網路文件的人很困擾，如果你要打 'AT&T' ，你必須要寫成'`AT&amp;T`' ，你還必須要轉換網址內的 `&` 符號，如果你要連結到：
...
不過要注意的是，code 範圍內，不論是行內還是區塊， `<` 和 `&` 兩個符號都 *一定* 會被轉換成 HTML 實體，這項特性讓你可以很容易的用 Markdown 寫 HTML code （和 HTML 相對而言， HTML 語法中，你要把所有的 `<` 和 `&` 都轉換為 HTML 實體，你才能在 HTML 文件裡面寫出 HTML code。）
...
是的，這確實讓你要花比較多功夫插入 `<br />` ，但是「每個換行都轉換為 `<br />`」的方法在 Markdown 中並不適合， Markdown 的 email 式的 [區塊引言][bq] 和多段落的[清單][l] 在使用換行來排版的時候，不但更好用，還更好閱讀！
...
和程式相關的寫作或是標籤語言原始碼通常會有已經排版好的程式碼區塊，通常這些區塊我們並不希望它照一般段落文件的方式去排版，而是照原來的樣子顯示，Markdown 會用 `<pre>` 和 `<code>` 標籤來把程式碼區塊包起來。
